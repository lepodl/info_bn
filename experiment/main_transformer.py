...
import data as dad
from models import transformer as mdl
...
# get_batch subdivides the source wikitext into chunks of length args.bptt.
# If source is equal to the example output of the batchify function, with
# a bptt-limit of 2, we'd get the following two Variables for i = 0:
# ┌ a g m s ┐ ┌ b h n t ┐
# └ b h n t ┘ └ c i o u ┘
# Note that despite the name of the function, the subdivison of wikitext is not
...
def batchify(data, bsz, device):
    ...
    # Evenly divide the wikitext across the bsz batches.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='../data/wikitext/wikitext-2',
                        help='location of the wikitext corpus')
    ...
    parser.add_argument('--init_var', type=float, default=1,
                        help='weights are initialized with variance init_var/ninp')  # include the linear and q, k, v.
    ...
    corpus = dad.Corpus(args.data)

    # Starting from sequential wikitext, batchify arranges the dataset into columns.
    ...
    def train(optimizer, epoch):
        ...
        for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
            ...
            if args.tensorboard:
                writer.add_scalar(f"train_loss", loss, i + epoch * len(train_data))
    ...
    if args.save_base_shapes:
        ...
        if args.deferred_init:
            ...
            base_shapes = get_shapes(
                deferred_init(mdl.TransformerModel, args, ntokens, ninp=args.d_model, nhead=args.nhead,
                              nhid=args.d_model * args.ffn_ratio, nlayers=args.nlayers, dropout=args.dropout,
            ...
            delta_shapes = get_shapes(
                ...
                deferred_init(mdl.TransformerModel, args, ntokens, ninp=args.d_model * 2, nhead=args.nhead,
            ...
            base_shapes = get_shapes(
                mdl.TransformerModel(args, ntokens, ninp=args.d_model, nhead=args.nhead,
                                     nhid=args.d_model * args.ffn_ratio, nlayers=args.nlayers, dropout=args.dropout,
                                     tied=args.tied, bias=args.bias, encoder_var=args.init_var,
                                     decoder_var=args.init_var, standparam=args.load_base_shapes == '')
            )
            delta_shapes = get_shapes(
                # just need to change whatever dimension(s) we are scaling
                mdl.TransformerModel(args, ntokens, ninp=args.d_model * 2, nhead=args.nhead,
                                     nhid=args.d_model * args.ffn_ratio * 2,
                                     nlayers=args.nlayers, dropout=args.dropout,
                                     tied=args.tied, bias=args.bias, encoder_var=args.init_var,
                                     decoder_var=args.init_var, standparam=args.load_base_shapes == '')
        ...
        sys.exit()
    model = mdl.TransformerModel(args, ntokens, ninp=args.d_model, nhead=args.nhead, nhid=args.d_model * args.ffn_ratio,
                                 nlayers=args.nlayers, dropout=args.dropout,
                                 tied=args.tied, bias=args.bias, encoder_var=args.init_var,
                                 decoder_var=args.init_var, standparam=args.load_base_shapes == '')  # decode_var invalid
    if args.load_base_shapes:
    ...
    # implement a scheduler/ maybe should close in muP case.
    ...
    try:
        ...
        for epoch in range(start_epoch + 1, args.epochs + 1):
            ...
            if args.tensorboard:
                ...
                writer.add_scalar(f"ppl", np.exp(val_loss), epoch)
            print('-' * 89)
